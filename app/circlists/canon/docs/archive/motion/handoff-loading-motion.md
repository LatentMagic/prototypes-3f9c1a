...
This session supersedes parts of `handoff-brand-motion.md` — read that one for the still-valid decisions it lists (rail uses `PulseLockup` not the bare mark; pulse lives on the rail not auth; the two inline-button spinners stay the plain ring). Its spinner *specifics* (32px, captions, four separate loading views, slate billing bg) are now out of date — this doc is the current truth for those.
...
- `handoff-brand-motion.md` — prior session; still-valid decisions, stale spinner specifics.
