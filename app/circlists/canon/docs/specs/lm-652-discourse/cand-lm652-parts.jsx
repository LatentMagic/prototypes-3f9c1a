...
// The way out of an EDIT (item 4, ratified 2026-08-19; placement chosen from
// playground/wb-edit-commit.html, option 03, 2026-08-19). Only an edit box needs
// one: clearing the field does not give the original words back. It stands in the
// name row, where the ⋯ that opened the edit was — NOT on the field's edge, which
// already carries the count and the arrow, so the one committing control keeps
// that slot to itself. The mark exists nowhere else in the product.
const CandCrossGlyph = ({ size = 16 }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" aria-hidden="true" style={{ display: 'block' }}>
    <line x1="6.5" y1="6.5" x2="17.5" y2="17.5" /><line x1="17.5" y1="6.5" x2="6.5" y2="17.5" />
  </svg>
);

const CandEditOut = ({ onClick }) => (
  <button type="button" className="cand-editout" onClick={onClick} aria-label="Discard this edit" title="Discard this edit"
    style={{ marginLeft: 'auto', alignSelf: 'center', flexShrink: 0, width: 34, height: 34, padding: 0, border: 0,
      background: 'transparent', borderRadius: 'var(--radius-md)', cursor: 'pointer',
    ...
    <div style={{ position: 'relative', background: CAND_PAPER.bg, border: '1px solid ' + (focus ? 'var(--color-accent)' : CAND_PAPER.bd), borderRadius: 'var(--radius-md)', padding: '10px 12px 4px', transition: 'border-color var(--duration-base)' }}>
      <textarea ref={ref} className="cand-write" value={value} maxLength={max} placeholder={placeholder} aria-label={ariaLabel}
        ...
        style={{ display: 'block', width: '100%', border: 0, outline: 'none', background: 'transparent', resize: 'none', padding: 0,
          ...
          paddingRight: gutter }} />
      <div aria-hidden="true" style={{ display: 'flex', justifyContent: 'flex-end', minHeight: 15, paddingRight: gutter }}>
...
// speaking in the conversation (a turn or a reply). The mark is NOT touched:
// enrolling is not reading, and the surface moves `talkSeenAt` forward when the
// member leaves. Marking read and placing a reaction never enrol.
...
// `watching` is kept as a defensive check; it can never fire on its own.
