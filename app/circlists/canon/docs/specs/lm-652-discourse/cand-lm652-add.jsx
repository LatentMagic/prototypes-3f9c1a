...
//   · the way back         — whiteboard 3, option 2 (2026-08-18): two named acts
//                           at the foot of the writing face — Discard quietly on
//                           the left, Add as the primary on the right. No glyph
//                           carries the exit, and the head keeps only the link.
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', width: '200%',
            ...
            <div style={{ width: '50%', flexShrink: 0 }} {...(face ? {} : { inert: '' })}>
              <div ref={writeFaceRef}>
                ...
                {/* The link's head row exists only when there is a link to name;
                    with an empty slot it would be a reserved blank, and the room
                ...
                {/* The two acts, named. Discard throws the words away and returns
                    to the link; Add commits the whole contribution from here. */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginTop: 'var(--space-4)', minHeight: 44 }}>
                  <button type="button" className="cand-quiet" onClick={() => { setThought(''); toLink(); }}
                    style={{ background: 'transparent', border: 0, cursor: 'pointer', padding: '10px 2px',
                      font: '500 14px/1.3 var(--font-sans)', color: 'var(--color-fg-3)' }}>Discard</button>
                  <Button type="button" variant="primary" onClick={commit}>Add</Button>
                </div>
              </div>
            </div>
          </div>
