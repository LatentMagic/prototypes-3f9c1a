...
- **The dormant circle is one screen for everyone.** The role branch is gone: every member
  of a sleeping circle sees the same thing — asleep, **Fund**, the ordinary contact door,
  and **Leave** — and it names nobody. Funding a dormant circle is open to any member, and
  the champion role passes to whoever funds it next, stated in copy on that screen. The
  suspended branch likewise loses its role and its Fund button, and says why funding is
  absent rather than leaving it inferred.
- **Leaving a circle exists.** A member leaves from beneath the roster or from the dormant
  screen, through the house destructive confirmation; the circle drops from the switcher and
  they land on their next default circle. The champion of a funded circle has no Leave — a
  statement stands in the control's slot, with cancel one click above it.
- **The cost of stepping back is carried before the act.** Cancel funding now confirms — the
  powers end with the paid period, the circle sleeps, the next funder champions it — before
  anything hands off to the provider. Nothing announces the role change afterwards.
- **Account deletion, and life after it.** Account settings carries delete, with one
  confirmation that enumerates no circles. A circle whose champion is gone keeps working to
  the end of its paid period with **no management offered to anybody**, and a statement
  where the crown was. The funding card gains a third textual state for a renewal being
  retried, carrying its retry window as a duration.
