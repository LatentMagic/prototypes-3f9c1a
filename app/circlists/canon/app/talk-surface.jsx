          ...
          {(kids.length > 0 || replyTo === t.id) && (
            <div style={rail}>
              ...
              {/* The foot of the group. Reply sits here, where the member is once
                  ...
                  Reply STANDS IN EVERY STATE (item 1, ratified 2026-08-19),
                  including while a tail is held back: nothing on the surface may
                  disappear as a side effect of a collapse, and Reply is not the
                  collapse's business. Pressing it while replies are held back does
                  two things in one press — the tail unfolds, then the composer
                  opens beneath it — so nobody composes against words they were
                  never shown. */}
              {kids.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                  ...
                  <button type="button" className="cand-quiet" style={candQuietBtn}
                    onClick={() => {
                      if (replyTo === t.id) { setReplyTo(null); setReply(''); return; }
                      if (rest > 0) setShown(s => ({ ...s, [t.id]: true }));
                      setReplyTo(t.id); setReply('');
                    }}>Reply</button>
                </div>
              )}
              {replyTo === t.id && (
