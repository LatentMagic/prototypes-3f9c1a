...
Built `docs/specs/lm-652-discourse/discourse-playground.html` (+ five `pg-disc-*.jsx` modules) from a
...
- `docs/specs/lm-652-discourse/README.md` — the option table, the settled constraints, and the note that the
  static disc is a copy of the shipped Swell geometry.
- `GOTCHA.md` #2 and #5 — both bit during this session (see Learnings).

## Recent changes

New folder `docs/specs/lm-652-discourse/`:
...
- `docs/specs/lm-652-discourse/discourse-playground.html` (+ `pg-disc-data|parts|card|places|app.jsx`)
- `docs/specs/lm-652-discourse/discourse-playground-standalone.html` — self-contained, 1.7 MB.
  Compiled output: edit the sources and re-run the bundler; never edit it directly.
- `docs/specs/lm-652-discourse/README.md`
...
- **The brief is not in the repo.** It was pasted into chat as "Circlists — discourse playground"
  and is summarised in `docs/specs/lm-652-discourse/README.md`. If it matters, ask the user to re-paste it
  and save it as `docs/specs/lm-652-discourse/PROMPT.md` (the convention `docs/specs/biz-80-metadata/`
  follows).
