...
// The band: the thought card's face while it is behind. A thought that fits on
// the band whole is READ THERE — the band is inert, has no chevron, and there is
// nothing to bring forward, because nothing is being held back. Only a thought
// the band cannot finish is openable, and the chevron is what says so. Measured,
// never guessed from a character count.
// The mark at the right of the band, saying there is more held back than the
// band can show. Which mark is the open question; the row takes it as a prop.
...
const CandBandFace = ({ item, onOpen, setHeld, held, innerRef, measurable, mark }) => {
  ...
  if (!held) return <div ref={innerRef} style={pad}>{words}</div>;
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div className={'cand-paper ' + cls} ref={paperBox}
        style={{ position: 'relative', zIndex: front ? 2 : 0,
          ...
          height: (open ? altH : bandH) || 'auto', overflow: 'hidden' }}>
        <div className="cand-face" aria-hidden={open} style={{ position: 'absolute', inset: '0 0 auto 0', opacity: open ? 0 : 1, pointerEvents: open ? 'none' : 'auto' }}>
          <CandBandFace item={item} held={held} setHeld={setHeld} mark={mark} measurable={!open && settled} innerRef={bandRef} onOpen={() => swap(true)} />
        </div>
