...
**Files:** `app/liveliness.jsx`, `app/main.jsx`, `app/shell.jsx`, `app/config.jsx`, `circlists.html`
...
**#618 — the browser tab** — not built. There is no browser tab in this posture (the prototype runs
inside a preview frame), so the signal cannot be exercised or reviewed here. The decided design still
lives in `tab-arrival-signal-pack/` — geometry, the fail-closed Safari branch, the rejected
alternatives — and the arrival paths in `main.jsx` (`landItem` / `queueItem`) are the single place a
tab signal would hook into if it is ever picked up on a real surface.
...
per-browser. The whole of #618 likewise — no tab, no favicon, no background focus to test against.
