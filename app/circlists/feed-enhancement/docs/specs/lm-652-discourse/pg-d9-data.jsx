// ============================================================================
// Discourse v9 — seeded circle + the five states' declarations.
...
// ============================================================================
const { M: D9M, IT: D9IT } = window.CircSeed;
const [D9_HEART, D9_FIRE, D9_THUMB, D9_BULB, D9_LOL] = window.RX_GLYPHS;
...
const T = (by, text, at, glyph, intensity, replyTo) =>
  ({ id: 't' + (++d9n), by, text, at, glyph: glyph || null, intensity: intensity == null ? null : intensity, replyTo: replyTo || null });

const D9_MEMBERS = [
  D9M('You', 'sam.rivera@gmail.com'), D9M('Priya N.', 'priya.n@example.com'),
  D9M('Marcus T.', 'marcus.t@example.com'), D9M('Ada L.', 'ada.l@example.com'),
  D9M('Dev K.', 'dev.k@example.com'), D9M('Lena P.', 'lena.p@example.com'),
  D9M('Nadia F.', 'nadia.f@example.com'), D9M('Theo B.', 'theo.b@example.com'),
  D9M('Sam R.', 'sam.r@example.com'),
];
...
const C = (url, by, read, at, opts = {}) => ({
  ...D9IT(url, by === 'You' ? 'Added by you' : 'Added by ' + by, read, opts.reactions || []),
  ...(D9_META[url] || {}),
  at,
...
const d9Silt = (from, to) => D9_SILT.slice(from, to).map(([url, by, days], n) => C(url, by, true, ago(days * DAY), {
  reactions: [
    { name: n % 2 ? 'Priya N.' : 'Marcus T.', glyph: n % 3 === 0 ? D9_THUMB : n % 3 === 1 ? D9_BULB : D9_HEART, intensity: 0.3 + (n % 4) * 0.15 },
...
const d9Cards = () => [
  ...
  C('https://martinfowler.com/articles/cd-pipeline.html', 'Sam R.', false, ago(2 * HR), {
    reactions: [{ name: 'Dev K.', glyph: D9_THUMB, intensity: 0.4 }],
  }),
  ...
  C('https://blog.rust-lang.org/2026/01/async-internals', 'Priya N.', false, ago(5 * HR), {
    ...
    reactions: [{ name: 'Marcus T.', glyph: D9_FIRE, intensity: 0.66 }],
  }),
  ...
  C('https://arxiv.org/abs/2503.04918', 'Marcus T.', false, ago(28 * HR), {
    ...
    reactions: [
      { name: 'Priya N.', glyph: D9_BULB, intensity: 0.82 }, { name: 'Ada L.', glyph: D9_THUMB, intensity: 0.5 },
      { name: 'Dev K.', glyph: D9_FIRE, intensity: 0.62 }, { name: 'Lena P.', glyph: D9_BULB, intensity: 0.44 },
      { name: 'Theo B.', glyph: D9_BULB, intensity: 0.28 }, { name: 'Marcus T.', glyph: D9_BULB, intensity: 0.6 },
      { name: 'Nadia F.', skipped: true },
    ],
    talk: (() => { const a = [
      T('Priya N.', 'The index itself is not the interesting part. The rebuild cost is.', ago(26 * HR), D9_BULB, 0.82),
      T('Ada L.', 'Benchmarks are on synthetic traces again. Nobody\u2019s series look like that.', ago(24 * HR), D9_THUMB, 0.5),
      T('Dev K.', '\u00a75 is the honest bit. Rare to see a paper publish the case where it loses.', ago(20 * HR), D9_FIRE, 0.62),
      T('Theo B.', 'Skimmed it. The related-work section is a decent reading list on its own.', ago(9 * HR), D9_BULB, 0.28),
      T('Marcus T.', 'Rereading. The rebuild cost is the point I would push on if we tried this here.', ago(4 * HR), D9_BULB, 0.6),
    ]; a.splice(3, 0, T('Lena P.', 'Agreed, though the trace generator is at least public this time.', ago(18 * HR), D9_BULB, 0.44, a[1].id)); return a; })(),
  }),
  ...
  C('https://newsletter.pragmaticengineer.com/p/scaling-on-call', 'Ada L.', false, ago(3 * DAY), {
    ...
    reactions: [{ name: 'Dev K.', glyph: D9_FIRE, intensity: 0.9 }, { name: 'Priya N.', glyph: D9_THUMB, intensity: 0.5 }],
    talk: [
      T('Dev K.', 'The rota section is the argument I failed to make last quarter.', ago(2.6 * DAY), D9_FIRE, 0.9),
      T('Priya N.', 'We would need to fix handover before any of this lands.', ago(2.2 * DAY), D9_THUMB, 0.5),
    ],
  }),
  ...
  (() => {
    const t = [
      T('Ada L.', 'The cancellation pattern here is the one I keep reaching for and never remember correctly.', ago(6 * DAY), D9_FIRE, 0.88),
      T('Dev K.', 'Half of our pipeline bugs are this, written badly.', ago(5 * DAY), D9_THUMB, 0.5),
      T('You', 'It reads dated until you notice that everything since is sugar over it.', ago(4 * DAY), D9_BULB, 0.55),
    ];
    t.push(T('Marcus T.', 'Sugar that changed who can write it, though. Errgroup put this pattern in reach of the whole team.', ago(5 * HR), D9_BULB, 0.7, t[2].id));
    t.push(T('Lena P.', 'Sent this to my old team. Two of them have rewritten a worker on the back of it.', ago(90 * MIN), D9_HEART, 0.34));
    return C('https://go.dev/blog/pipelines', 'You', true, ago(8 * DAY), {
      ...
      reactions: [
        { name: 'Ada L.', glyph: D9_FIRE, intensity: 0.88 }, { name: 'Dev K.', glyph: D9_THUMB, intensity: 0.5 },
        { name: 'You', glyph: D9_BULB, intensity: 0.55 }, { name: 'Marcus T.', glyph: D9_BULB, intensity: 0.7 },
        { name: 'Lena P.', glyph: D9_HEART, intensity: 0.34 }, { name: 'Priya N.', skipped: true },
      ],
  ...
  (() => {
    const t = [
      T('Marcus T.', 'The part where she simply reads the packets. I have been guessing at this for years.', ago(9 * DAY), D9_LOL, 0.86),
      T('You', 'This is going into the onboarding doc.', ago(9 * DAY), D9_FIRE, 0.5),
    ];
    t.push(T('Ada L.', 'Same. I had built a whole mental model out of folklore.', ago(8 * DAY), D9_BULB, 0.62, t[0].id));
    t.push(T('Nadia F.', 'The resolver diagram is the only one I have ever been able to hold in my head.', ago(2 * HR), D9_BULB, 0.72));
    return C('https://jvns.ca/blog/2026/02/dns-resolvers/', 'Priya N.', true, ago(10 * DAY), {
      ...
      reactions: [
        { name: 'Marcus T.', glyph: D9_LOL, intensity: 0.86 }, { name: 'You', glyph: D9_FIRE, intensity: 0.5 },
        { name: 'Ada L.', glyph: D9_BULB, intensity: 0.62 }, { name: 'Nadia F.', glyph: D9_BULB, intensity: 0.72, at: ago(2 * HR) },
  ...
  C('https://www.kernel.org/doc/html/latest/process/submitting-patches.html', 'Sam R.', true, ago(12 * DAY), {
    reactions: [
      { name: 'Lena P.', glyph: D9_THUMB, intensity: 0.3 }, { name: 'Priya N.', glyph: D9_BULB, intensity: 0.66 },
      { name: 'You', skipped: true },
    ],
    talk: [
      T('Lena P.', 'Dry, but every rejection I have ever had is in here somewhere.', ago(11 * DAY), D9_THUMB, 0.3),
      T('Priya N.', 'Section 3 explains our last three bounced patches.', ago(6 * DAY), D9_BULB, 0.66),
    ],
    seenAt: ago(6 * DAY),
  }),
  ...d9Silt(9, 12),
  ...
  (() => {
    const t = [
      T('Priya N.', 'The simple/easy split has changed how I argue in design reviews more than any book has.', ago(20 * DAY), D9_FIRE, 0.92),
      T('Ada L.', 'Watched it again on the train. It is a different talk at thirty than it was at twenty-five.', ago(19 * DAY), D9_FIRE, 0.86),
      T('Dev K.', 'I come out of this wanting to delete something, every time.', ago(18 * DAY), D9_BULB, 0.6),
      T('You', 'The complecting section is the only vocabulary I have found that makes the argument without sounding rude.', ago(17 * DAY), D9_FIRE, 0.62),
      T('Lena P.', 'It is the pacing. He never once rushes you.', ago(16 * DAY), D9_HEART, 0.55),
      T('Theo B.', 'Fine, but the Clojure framing dates it in places.', ago(14 * DAY), D9_THUMB, 0.3),
    ];
    t.push(T('Nadia F.', 'Ignore the Clojure and it holds anyway. The examples are incidental to the argument.', ago(13 * DAY), D9_BULB, 0.5, t[5].id));
    t.push(T('Marcus T.', 'It is the only talk I can send to a manager and to an intern and have both get something out of it.', ago(12 * DAY), D9_FIRE, 0.78, t[3].id));
    ...
    t.push(T('Sam R.', 'First watch. I understand why this keeps coming up.', ago(7 * HR), D9_BULB, 0.8));
    ...
    return C('https://www.youtube.com/watch?v=Kx7Bvksk_qg', 'Marcus T.', true, ago(22 * DAY), {
      ...
      reactions: [
        { name: 'Priya N.', glyph: D9_FIRE, intensity: 0.92 }, { name: 'Ada L.', glyph: D9_FIRE, intensity: 0.86 },
        { name: 'Dev K.', glyph: D9_BULB, intensity: 0.6 }, { name: 'You', glyph: D9_FIRE, intensity: 0.62 },
        { name: 'Lena P.', glyph: D9_HEART, intensity: 0.55 }, { name: 'Theo B.', glyph: D9_THUMB, intensity: 0.3 },
        { name: 'Nadia F.', glyph: D9_BULB, intensity: 0.5 }, { name: 'Marcus T.', glyph: D9_FIRE, intensity: 0.78 },
        { name: 'Sam R.', glyph: D9_BULB, intensity: 0.8 },
      ],
  ...
  C('https://sqlite.org/whentouse.html', 'former member', true, ago(26 * DAY), {
    reactions: [
      { name: 'Priya N.', glyph: D9_THUMB, intensity: 0.5 }, { name: 'You', glyph: D9_THUMB, intensity: 0.44 },
  ...
  C('https://go.dev/blog/errors-are-values', 'Ada L.', true, ago(30 * DAY), {
    reactions: [{ name: 'Priya N.', glyph: D9_THUMB, intensity: 0.5 }, { name: 'You', skipped: true },
      { name: 'Theo B.', glyph: D9_BULB, intensity: 0.66, at: ago(4 * HR) }],
    talk: [
      T('Priya N.', 'The scanner example finally made errcheck stop feeling like a chore.', ago(29 * DAY), D9_THUMB, 0.5),
      T('Theo B.', 'Came to this a month late. The scanner example is the whole argument in twelve lines.', ago(4 * HR), D9_BULB, 0.66),
    ],
...
const d9Circle = () => ({
  ...
  members: D9_MEMBERS, items: d9Cards(), pending: [], queued: [], unseen: false, lastSeenAt: NOW,
});
...
window.PGD9Data = { d9Circle, d9Cards, D9_MEMBERS, D9_DROP, T, NOW, HR, DAY, MIN };
