// ============================================================================
// BIZ-136 — leaving a circle. Build. Writes both artefacts from one module list:
//
//   leaving-a-circle-playground.html             the working rig, Babel-based,
//                                                at the project root so app/*
//                                                and tokens.css resolve.
//   leaving-a-circle-playground-standalone.html  the asset: compiled, pure
//                                                HTML, no transpiler, no
//                                                external fetch except fonts.
//
...
//
// Run from the candidate build root:
//   node docs/specs/biz-136-leaving-a-circle/build.mjs
// Vendor files (Babel, React UMD) are fetched once into .build-cache/ beside
// this script and reused. Structure and vendoring follow the BIZ-136 whiteboard
// build (docs/specs/biz-136-mobile-chrome/build.mjs) — the differences are the
// module list, the titles and the filenames.
// ============================================================================
...
const REL = 'docs/specs/biz-136-leaving-a-circle';

// The app's own load order, trimmed to what this rig mounts, then the rig.
const MODULES = [
  ...
  'app/home.jsx', 'app/app-shell.jsx', 'app/spaces.jsx', 'app/invite-link.jsx',
  ...
  `${REL}/pg-lc-app.jsx`, `${REL}/pg-lc-chrome.jsx`, `${REL}/pg-lc-rig.jsx`,
];
...
// the modules), then the rig's own.
const appStyle = () => {
...
const HEAD_TITLE = 'Leaving a circle (BIZ-136)';
...
`;
  fs.writeFileSync(path.join(ROOT, 'leaving-a-circle-playground.html'), html);
  return html.length;
}
...
// app/feed.jsx falls back to Google's live favicon service for any host
// app/seed-data.jsx has no local file for — an external fetch the standalone
// may not make. Every host in the seed is resolved ONCE here, at build time,
// and baked in, so the fallback never fires.
...
`;
  fs.writeFileSync(path.join(ROOT, 'leaving-a-circle-playground-standalone.html'), html);
