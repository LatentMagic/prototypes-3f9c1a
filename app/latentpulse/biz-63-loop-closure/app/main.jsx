...
const LPApp = () => {
  ...
  const markRead = (item, myRx) => setSpaces(prev => prev.map(s => s.id === currentId ? { ...s, items: s.items.map(i => i.id === item.id
    ? { ...i, read: true, reactions: myRx ? [...(i.reactions || []), myRx] : (i.reactions || []) }
    : i) } : s));
  ...
  const onConfirm = () => {
    if (!confirm) return;
    // Mark-as-read confirm is unchanged; on confirm it hands off to the reaction
    // moment (input → reveal). The read itself is committed inside that flow.
    if (confirm.kind === 'mark-read') { setReactFlow({ item: confirm.item }); setConfirm(null); return; }
        ...
        {rxSwitch && (
          <div className="lp-launcher-panel" role="menu" style={{ left: 0, right: 'auto', bottom: 'calc(100% + 8px)', top: 'auto', width: 264 }}>
            ...
            {RX_VARIANTS.map((v) => {
              ...
              return (
                <button key={v.key} className="lp-launcher-item" onClick={() => { setRxVariant(v.key); setRxSwitch(false); }}
                  style={{ display: 'flex', alignItems: 'baseline', gap: 8, background: on ? 'var(--color-surface-sunken)' : 'transparent' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: on ? 'var(--color-accent)' : 'var(--color-fg-3)', width: 14 }}>{v.n}</span>
                  <span style={{ fontWeight: on ? 600 : 500 }}>{v.name}</span>
                  <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--color-fg-3)', fontWeight: 400 }}>{v.sub}</span>
                </button>
              );
            })}
          </div>
