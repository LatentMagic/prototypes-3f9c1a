...
// ---- Empty states — typographic destinations, no illustration -------------
const EMPTY_COPY = {
  'active-caught-up': {
    primary: "You're all caught up.",
    supporting: 'Nothing left to read here.',
  },
  'active-fresh': {
    primary: 'Nothing here yet.',
    supporting: 'Add the first link and it lands in everyone\u2019s queue.',
  },
  'read-nothing': {
    primary: 'Nothing read yet.',
    supporting: 'Items you mark as read collect here.',
  },
};
const EmptyState = ({ register, cta }) => {
  const c = EMPTY_COPY[register];
  return (
    <div style={{
...
const AddReveal = ({ open, isMobile, onClose, onAdd }) => {
  ...
  return (
    <>
      <div onClick={onClose} className={isMobile ? 'lp-anim-fade' : ''} style={{
        position: 'fixed', inset: 0, zIndex: 120,
        background: isMobile ? 'var(--color-scrim)' : 'transparent',
      }} />
      <form role="dialog" aria-label="Add a link" onSubmit={submit} className={isMobile ? 'lp-anim-sheet' : 'lp-anim-fade'} style={surface}>
