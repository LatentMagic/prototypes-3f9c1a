// ============================================================================
// LatentPulse v0.2.1 — root state machine (per-space billing).
...
const LPApp = () => {
  ...
  const finishOtc = () => {
    if (postAuthTo === 'post-signup') {
      // Land with NO spaces → create the first one (creating is funding).
      setSpaces([]); setCurrentId(null); setRoute('create-space');
    } else {
      if (spaces.length === 0) setRoute('create-space'); else enterSpace(currentId || spaces[0]?.id);
    }
  };
  ...
  const SCENARIOS = [
    ...
    { k: 'Create + fund a space', go: () => setRoute('create-space') },
  ];
  ...
  const inShell = (content, opts = {}) => (
    <AppShell
      ...
      onSelectSpace={enterSpace} onCreateSpace={() => setRoute('create-space')}
  ...
  if (route === 'signin') {
    ...
    screen = <GoogleReturn onDone={() => { if (postAuthTo === 'post-signup') { setSpaces([]); setCurrentId(null); setRoute('create-space'); } else enterSpace(currentId || 'sp-backend'); }} />;
  } else if (route === 'recovery') {
    screen = <Recovery onDone={() => { if (spaces.length === 0) setRoute('create-space'); else enterSpace(currentId || 'sp-backend'); }} onBackToSignin={() => setRoute('signin')} />;
  } else if (route === 'funding') {
    screen = <FundingPage user={user} spaceName={fundFlow.name} mode={fundFlow.mode}
      onFund={() => setRoute('checkout')}
      onCancel={() => { if (fundFlow.mode === 'refund') enterSpace(fundFlow.spaceId); else if (spaces.length > 0) enterSpace(currentId || spaces[0]?.id); else setRoute('create-space'); }} />;
    ...
    screen = <CreateSpace onCreate={beginCreateFund} canCancel={spaces.length > 0} onCancel={() => enterSpace(currentId || spaces[0]?.id)} />;
    ...
    screen = inShell(<MembersSurface space={space} isChampion={isChampion(space)} championName={space ? space.champion : ''}
      onBack={() => enterSpace(currentId)} onInvite={inviteEmail} onManageFunding={openManageFunding} />);
  } else if (route === 'account') {
    screen = inShell(<AccountSettings user={user} onBack={() => enterSpace(currentId)} />);
  } else {
    ...
    if (space && !space.funded) {
        ...
        <main style={{ flex: 1, width: '100%' }}>
          <div style={{ maxWidth: 'var(--max-feed-width)', margin: '0 auto', padding: isMobile ? '16px 16px 112px' : '28px 24px 120px', width: '100%' }}>
            {loadingFeed ? <FeedLoading />
              : visible.length === 0 ? (
                tab === 'active'
                  ? <EmptyState register={space && space.items.length === 0 ? 'active-fresh' : 'active-caught-up'}
                      cta={space && space.items.length === 0 ? <Button variant="primary" icon={<Icon name="plus" size={16} color="#fff" strokeWidth={2} />} onClick={() => setAddOpen(true)}>Add a link</Button> : null} />
                  : <EmptyState register="read-nothing" />
              ) : (
      ...
      screen = inShell(
        <>
          <Tabs active={tab} onChange={setTab} activeCount={activeItems.length} readCount={readItems.length} />
          {feed}
  ...
  return (
    <>
      ...
      <div className="lp-launcher-wrap" style={launchWrapStyle}>
        {launcher && (
