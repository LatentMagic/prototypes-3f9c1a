...
const FundingPage = ({ spaceName, mode = 'new', onFund, onCancel, user }) => {
  ...
  return (
    <div style={{ minHeight: 'var(--lp-vh)', background: 'var(--color-canvas)', display: 'flex', flexDirection: 'column' }}>
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px' }}>
        <Wordmark size={16} />
        {onCancel && <TextLink onClick={onCancel}>Cancel</TextLink>}
      </header>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 20px 64px' }}>
        <div style={{ maxWidth: 468, width: '100%', textAlign: 'center' }}>
          ...
          }}>{refund ? 'Returning champion' : 'New space'}{spaceName ? ' \u00b7 ' + spaceName : ''}</div>
          <h1 style={{
...
const Checkout = ({ user, spaceName, refund, onSuccess, onCancel }) => {
  ...
  const inp = {
    width: '100%', boxSizing: 'border-box', fontFamily: 'var(--font-mono)', fontSize: 15,
